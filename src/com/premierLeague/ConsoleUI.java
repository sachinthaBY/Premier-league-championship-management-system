...
public class ConsoleUI {
    ...
    public static void main (String[] args) {
        ...
        do {
            ...
            switch (guess) {
                case 1:
                    console.addClub(); //add a club, if user input 1
                    break;
                case 2:
                    console.dltClub(); //delete a club, if user input 2
                    break;
                case 3:
                    console.displayTable(); //display club name, points and goal difference, if user input 3
                    break;
                case 4:
                    console.displayClub(); //display the relevant club stats, if user input 4
                    break;
                case 5:
                    console.addMatch(); //adding a match if user input 5
                    break;
                case 6:
                    console.showGUI(); //display GUI, if user input 6
                    break;
    ...
    private void addClub(){
        ...
        System.out.println("Numbers of members : "); //getting numbers of the members in each clubs
    ...
    private void dltClub(){
        ...
        if(premierLeagueManager.clubList.size() > 0 ) {
            ...
            if (result) {
                System.out.println("Removed Successfully"); //successfully message
            } else {
    ...
    private  void displayTable(){
        ...
        if(premierLeagueManager.clubList.size() > 0 ) {
            ...
            String leftAlignFormat = "| %-15s | %-6s | %-6s |%n"; //setting alignment
            ...
            System.out.println("Please enter the teams first"); //if team is not in a club
        }
    ...
    private void displayClub(){
        ...
        if(premierLeagueManager.clubList.size() > 0 ) {
            ...
            FootballClub selectedTeam = premierLeagueManager.getTeam(clubID); // premierLeagueManager's clubId add to the selected team
    ...
    private void addMatch(){
        ...
        if(premierLeagueManager.clubList.size() > 1 ) {
            ...
            while(!premierLeagueManager.checkTeamById(id1)) {
                System.out.println("Enter the team 1 ID: "); //getting the team 1 id
                id1 = scan.next();
                if(!premierLeagueManager.checkTeamById(id1))
                    System.out.println("Team ID is invalid"); // if it is invalid
            }

            System.out.println("Enter the score of the team 1: "); //getting the score of team 1
            String score1 = scan.next();

            String id2 = null;
            while(!premierLeagueManager.checkTeamById(id2)) {
                System.out.println("Enter the team 2 ID: "); //get team 2 id
                id2 = scan.next();
